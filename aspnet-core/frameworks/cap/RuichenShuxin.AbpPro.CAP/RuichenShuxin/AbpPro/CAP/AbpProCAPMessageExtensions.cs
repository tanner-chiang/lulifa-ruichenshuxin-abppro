...
public static class AbpProCAPMessageExtensions
{
    ...
    public static bool TryGetTenantId(
        ...
        out Guid? tenantId)
    {
        if (message.Headers.TryGetValue(AbpProCAPHeaders.TenantId, out string tenantStr))
        {
    ...
    {
        return message.Headers.TryGetValue(AbpProCAPHeaders.CorrelationId, out correlationId);
    }
    /// <summary>
    /// 获取消息标头中的链路标识
    /// </summary>
