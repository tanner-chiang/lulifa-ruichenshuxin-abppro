...
public class CAPDistributedEventBus : DistributedEventBusBase, IDistributedEventBus
{
    ...
    protected virtual async Task PublishToCapAsync(string eventName, object eventData, Guid? messageId, string correlationId = null)
    {
        ...
        {
            headers.TryAdd(AbpProCAPHeaders.MessageId, messageId.ToString());
        }
        if (CurrentUser.Id.HasValue)
        {
            headers.TryAdd(AbpProCAPHeaders.UserId, CurrentUser.Id.ToString());
        }
        if (CurrentTenant.Id.HasValue)
        {
            headers.TryAdd(AbpProCAPHeaders.TenantId, CurrentTenant.Id.ToString());
        }
        if (!CurrentClient.Id.IsNullOrWhiteSpace())
        {
            headers.TryAdd(AbpProCAPHeaders.ClientId, CurrentClient.Id);
        }
        if (!correlationId.IsNullOrWhiteSpace())
        {
            headers.TryAdd(AbpProCAPHeaders.CorrelationId, correlationId);
        }
