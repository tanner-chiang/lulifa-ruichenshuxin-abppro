...
public class AbpProExceptionHandlingWrapperMiddleware : IMiddleware, ITransientDependency
{
    private readonly ILogger<AbpProExceptionHandlingWrapperMiddleware> _logger;

    private readonly Func<object, Task> _clearCacheHeadersDelegate;

    public AbpProExceptionHandlingWrapperMiddleware(ILogger<AbpProExceptionHandlingWrapperMiddleware> logger)
    {
        _logger = logger;
    ...
    private async Task HandleAndWrapException(HttpContext httpContext, Exception exception)
    {
        ...
        var wrapOptions = httpContext.RequestServices.GetRequiredService<IOptions<AbpProWrapperOptions>>().Value;
