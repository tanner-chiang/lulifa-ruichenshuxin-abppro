...
public class WrapResultChecker : IWrapResultChecker, ISingletonDependency
{
    ...
    public WrapResultChecker(IOptionsMonitor<AbpProWrapperOptions> optionsMonitor)
    {
    ...
    protected virtual bool CheckForBase(FilterContext context)
    {
        ...
        // 用户传递不包装
        if (context.HttpContext.Request.Headers.ContainsKey(AbpProHttpWrapConsts.AbpDontWrapResult))
        {
            return false;
        }

        // 用户传递需要包装
        if (context.HttpContext.Request.Headers.ContainsKey(AbpProHttpWrapConsts.AbpWrapResult))
        {
            return true;
        }
